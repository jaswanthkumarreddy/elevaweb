...
class Api extends API_Controller {
	...
	public function registration(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$this->form_validation->set_rules('password', lang('customers input password'), 'required|trim|min_length[1]|max_length[16]');  
	...
	public function editRegistration(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			//$this->form_validation->set_rules('email_id', lang('customers input email'), 'required|trim|valid_email|min_length[2]|max_length[100]|callback__check_email['.$this->input->post('email_id').']');	
			  
			//$this->form_validation->set_rules('password', lang('customers input password'), 'trim|min_length[8]|max_length[16]');  
   ...
   public function login()  { 
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if ($this->form_validation->run() == TRUE) {
				$login = $this->customers_model->login($this->input->post('email_id'), $this->input->post('password'),$this->input->post('site_address'));
				$results['success'] = true;
				$results['data'] = $login;				
				$results['message'] = 'User login successful.!';
			} else {
				$results['error'] = false;
				$results['message'] = 'Username and Password not match Or You are inactive!';
			}
		} else {
			$results['error'] = false;
			$results['message'] = 'Please select POST method.!';
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            if ($resultsData)
            {
               ...
               // $email_msg .= lang('core email end');
			    ...
			    $email_msg = '<p>Olá!
Sua senha Elevaweb foi renovada. Copie esta nova senha abaixo para acessar a Elevaweb . Caso queira, você pode criar uma nova senha acessando "Mudar Senha" no menu lateral Elevaweb do seu site.</p>
<p>'.$resultsData['new_password'].'</p><p>Qualquer dúvida entre em contato com nosso suporte@elevaweb.com.br
Obrigado</p>';
				...
				$from_email = 'suporte@elevaweb.com.br';
                $this->email->clear();
                $this->email->from($from_email, $this->settings->site_name);
                $this->email->reply_to($from_email, $this->settings->site_name);
