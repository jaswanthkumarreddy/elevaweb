...
class Customers_model extends CI_Model {
    ...
    function add_customer($data = array())
    {
        if ($data)
        {
			...
			if ($customer_id = $this->db->insert_id()) {
				...
				$up_data = array_merge($data,array('customer_id'=>$customer_id));
                return $up_data;
            }			
        }
        return FALSE;
    ...
    function edit_customer($data = array()){
        if($data){
			...
			if(!empty($_POST['profile_picture'])){
				...
				$img = str_replace(' ', '+', $img);
				$profile_data = base64_decode($img);
				$name = 'profile_'.$data['id'].'.jpg';
				file_put_contents('user_profile/'.$name, $profile_data);
            ...
            {
				$profile_url = base_url().'user_profile/'.$name;
				//$up_data = array_merge($up_data,array('customer_id'=>$data['id'],'profile_url'=>$profile_url,'register_date'=>$data['register_date']));
				$up_data = array_merge($up_data,array('customer_id'=>$data['id'],'profile_url'=>$profile_url,'created_on'=>$_POST['register_date']));
                return $up_data;
            } 
        } 
    ...
    function email_exists($email_id,$current=NULL)
    {
        $sql = "
            SELECT customer_id
            FROM {$this->_db}
            WHERE email_id = " . $this->db->escape($email_id) . " AND deleted = 0";
	...
	*/
     ...
     * @return array|boolean
     */
    function login($email_id = NULL, $password = NULL, $site_address = NULL)
    {
        ...
        if ($email_id && $password)
        {
            $sql = "
                SELECT
					...
					customer_websites
                FROM customers
                WHERE email_id = " . $this->db->escape($email_id) . " AND is_active = 1 LIMIT 1 ";
            ...
            if ($query->num_rows())
            {
                ...
                { 
					...
					if(empty( $results['customer_websites'] )){
						if(!empty($site_address)){
							$site = array($site_address,date("Y-m-d h:i:s"));
							//$site['website_url'] = $site_address;
							//$site['date'] = date("Y-m-d h:i:s");
							$str = serialize($site);
							$sql_web = "UPDATE {$this->_db} SET customer_websites = '".$str."' WHERE customer_id = ".$results['customer_id']."";
							$this->db->query( $sql_web );
						}
					}else{
						if(!empty( $results['customer_websites'] ) ){
							$old_sites = unserialize( $results['customer_websites'] );
							if(!empty( $old_sites ) && is_array( $old_sites )){
								if( !in_array( $site_address,$old_sites ) ){
									$site = array($site_address,date("Y-m-d h:i:s"));
									//$site['website_url'] = $site_address;
									//$site['date'] = date("Y-m-d h:i:s");
									$str = array_merge($old_sites,$site);
									$array_sites = serialize($str);
									$sql_web = "UPDATE {$this->_db} SET customer_websites = '".$array_sites."' WHERE customer_id = ".$results['customer_id']."";
									$this->db->query( $sql_web );
								}
							}
						}
					...
					if(!empty( $results['profile_picture'] )){
						$image_path = base_url().'user_profile/'.$results['profile_picture'];
					}else{
						$image_path = '';
					}
					
					$results = array_merge($results,array('profile_url'=>$image_path));
                    return $results;
                }
            }
        }
